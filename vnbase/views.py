...
def mvn(request, param):
    ...
    if vpn_link:
        ...
        context = {"date_name_daoqi": date_name_daoqi, "date_name2_vx": date_name2_vx}
...
def zfbvn(request, param):
    ...
    # 根据请求参数字符串去VpnLink的code对比,并且status=1，返回对应的link
    # 从数据库中获取对应的VpnLink对象
    vpn_link = VpnZfbOrder.objects.filter(code=param).first()
    if vpn_link:
        if vpn_link.status == 1 or vpn_link.status == 0:
            if vpn_link.status == 0:
                ...
                expiration_time = vpn_link.created_at + timedelta(days=vpn_link.month * 30)
                # 更新过期时间
